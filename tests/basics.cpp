...
TEST_CASE("constexpr values") {
    static_assert( att::traits::is_aggregate<person>);
    static_assert(!att::traits::is_aggregate<wrapper>);

    static_assert(att::traits::arity_of<person> == 2);
}
...
TEST_CASE("core functions and types") {
    ...
    att::tuple_type_of<person> tuple2 = tuple_refs;
    std::get<1>(tuple2) += 1;
    REQUIRE(p.age == 26);

    att::tuple_ref_type_of<person> tuple_refs2 = tuple_refs;
